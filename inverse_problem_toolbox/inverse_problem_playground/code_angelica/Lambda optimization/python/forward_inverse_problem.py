...
def classical_tikhonov(A,AA,L,LL,y,n_iterations=50):
    ...
    for index in range(0, n_iterations-1):
        ...
        #save
        lambdas_.append(lambda_test)
        errors_.append(error_term)
        magnitude_term_.append(magnitude_term)
        lambda_opt_.append(lambda_opt)
    ...
    return x_hat,lambda_opt, lambdas_,errors_,magnitude_term_,lambda_opt_
    ...
    for y_chunk in y_chunk_list:
        ...
        for index in range(0, n_iterations-1):       
            ...
            for i in range(0, np.size(lambda_test)):
                ...
                #inv_A_At= np.matmul(inv_A,np.transpose(A))
                #x_hat = np.matmul(inv_A_At,y);
                x_hat = np.matmul(np.matmul(np.linalg.inv(AA+lambda_test[i]*LL),np.transpose(A)),y_chunk);
                error_term[i]    = np.power(np.linalg.norm(np.matmul(A,x_hat)-y_chunk),2);
        ...
        x_hat_list.append(np.matmul(np.matmul(np.linalg.inv(AA+lambda_opt*LL),np.transpose(A)),y_chunk));
...
def classical_tikhonov_noiter(A,AA,L,LL,y,size_chunk=400):
    """
    Tikhonov global method reconstruction.
    The analytical solution of the inverse problem in terms of Tikhonov regularization is:
    phi_x_hat = inv(A'*A + lambda*L'*L)*A'*phi_T    
    ...
    for y_chunk in y_chunk_list:
        ...
        for j in range(0, lambda_test.shape[0]):
            x_hat = np.matmul(np.matmul(np.linalg.inv(AA+lambda_test[j]*LL),np.transpose(A)),y_chunk);
        ...
        x_hat = np.matmul(np.matmul(np.linalg.inv(AA+lambda_opt*LL),np.transpose(A)),y_chunk);
    ...
    for chunk in range(0,n_chunks):
        ...
        for iteration in range(0, n_iterations):
            ...
            for i in range(0, np.size(lambda_test_1)):
                for j in range(0, np.size(lambda_test_2)):
                    inv_term = np.linalg.inv(AA+lambda_test_1[i]*LL+lambda_test_2[j]*np.matmul(np.transpose(D),D))
                    sec_term = np.matmul(np.transpose(A),y_chunk_list[chunk])+lambda_test_2[j]*np.matmul(np.transpose(D),xref_chunk_list[chunk])
                    x_hat = np.matmul(inv_term,sec_term);
                    error_term_1[i,j] = np.power(np.linalg.norm(np.matmul(A,x_hat)-y_chunk_list[chunk]),2);
                    error_term_2[i,j] = np.power(np.linalg.norm(np.matmul(D,x_hat)-xref_chunk_list[chunk]),2);
                    magnitude_term[i,j]= np.power(np.linalg.norm(np.matmul(L,x_hat)),2);
        ...
        inv_term = np.linalg.inv(AA+lambda_opt[0]*LL+lambda_opt[1]*np.matmul(np.transpose(D),D))
        sec_term = np.matmul(np.transpose(A),y_chunk_list[chunk])+lambda_opt[1]*np.matmul(np.transpose(D),xref_chunk_list[chunk])
        x_hat = np.matmul(inv_term,sec_term);
