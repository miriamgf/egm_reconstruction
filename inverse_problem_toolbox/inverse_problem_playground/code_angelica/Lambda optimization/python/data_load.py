...
def load_egms(model='SR'):
    ...
    try:
        EG = np.transpose(np.array((h5py.File(data_path + model + '/signal.mat','r')).get('EG')))
    except:
        EG = scipy.io.loadmat(data_path + model + '/signal.mat').get('EG')
...
def load_egms_real(model):
    ...
    try:
        EGM = np.transpose(np.array((h5py.File(data_path_real + model + '/signals.mat','r')).get('EGM')))
        ECG = np.transpose(np.array((h5py.File(data_path_real + model + '/signals.mat','r')).get('ECG')))
    except:
...
def load_transfer():
    ...
    try:
        MTransfer = np.transpose(np.array((h5py.File(data_path + 'transfer.mat','r')).get('MTransfer')))
    except:
...
def load_transfer_real(model):
    ...
    try:
        A = np.transpose(np.array((h5py.File(data_path_real + model + '/transfer.mat','r')).get('A')))
    except:
...
def load_geometry_real(model):
    ...
    try:
        atrial_model_temp = np.array((h5py.File(data_path_real + model + '/geometry.mat','r')).get('atrial_model'))
        torso_model_temp = np.array((h5py.File(data_path_real + model + '/geometry.mat','r')).get('torso_model'))
    except:
        atrial_model_temp = scipy.io.loadmat(data_path_real + model + '/geometry.mat').get('atrial_model')
        torso_model_temp = scipy.io.loadmat(data_path_real + model + '/geometry.mat').get('torso_model')
...
def load_geometry():
    ...
    try:
        atrial_model_temp = np.array((h5py.File(data_path + 'geometry.mat','r')).get('atrial_model'))
        torso_model_temp = np.array((h5py.File(data_path + 'geometry.mat','r')).get('torso_model'))
    except:
        atrial_model_temp = scipy.io.loadmat(data_path + 'geometry.mat').get('atrial_model')
        torso_model_temp = scipy.io.loadmat(data_path + 'geometry.mat').get('torso_model')
...
def load_constrained_nodes():
    ...
    try:
        Cons_nodes = np.transpose(np.array((h5py.File(data_path + 'Constellation_nodes.mat','r')).get('constellation_nodes')))
    except:
        Cons_nodes = scipy.io.loadmat(data_path + 'Constellation_nodes.mat').get('constellation_nodes')
...
def load_constrained_nodes_real(model):
    ...
    try:
        Cons_nodes = np.transpose(np.array((h5py.File(data_path_real + model + '/EGM_mesh_index.mat','r')).get('EGM_mesh_index')))
    except:
        Cons_nodes = scipy.io.loadmat(data_path_real + model + '/EGM_mesh_index.mat').get('EGM_mesh_index')
