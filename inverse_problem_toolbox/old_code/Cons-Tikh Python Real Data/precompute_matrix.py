...
def precompute_matrix_real(A,atrial_model,model,order=0):
    ...
    if order == 0:
        ...
        # Load file with g1 and g2 L matrix.
        try:
            L = np.transpose(np.array((h5py.File(data_path_real + model + '/L_matrix.mat','r')).get('L_consg1')))
        except:
            L = scipy.io.loadmat(data_path_real + model + '/L_matrix.mat').get('L_consg1')
    elif order == 2:
        try:
            L = np.transpose(np.array((h5py.File(data_path_real + model + '/L_matrix.mat','r')).get('L_consg2')))
        except:
            L = scipy.io.loadmat(data_path_real + model + '/L_matrix.mat').get('L_consg2')
