...
def kuklik_DF_phase(signals,fs):
    ...
    pd = np.empty((n_signals,1)); pd[:] = np.nan; tvec = np.arange(1/fs,siglen/fs,1/fs) # Time at each sample in seconds
    seglength = 2*fs; n_overlap = fs; nFFT = np.power(200,2); # Zero pad FFT to get finer resolution (interpolates spectrum)
    
    for i in range(0, n_signals):
        # Find largest continuous chunk of signal for each channel
        isig = sig[i,:]; sigtimes = np.where(~np.isnan(isig))[0]; # Non-saturated times
        if np.size(sigtimes) > 0: # Make sure there is some signal here
            ...
            if np.size(sigseg)>=seglength: # Ensure we hace enough signal
                ...
                pdval = invf[np.where(pxx==np.max(pxx))[0]]; mind = np.argmin(np.abs(tvec-pdval))
    ...
    for i in range(0,n_signals):
        ...
        for t in range(0,siglen-1):
            if ~np.isnan(pd[i]):
                ...
                dvdt = diffv[i,t]; tt = np.arange(-halfpd,halfpd+1);
                swave = np.sin(2*np.pi*tt/pd[i])*np.abs(dvdt)*((1-np.sign(dvdt))/2)
                twave = t + tt; swave = np.delete(swave,np.where(np.logical_or(twave<0,twave>siglen-1)))
                twave = np.delete(twave,np.where(twave<0)); twave = np.delete(twave,np.where(twave>siglen-1))
...
def df_estimation_peak(input_signals,fs):
    ...
    f, pxx = sigproc.welch(signals[:,int(1*fs):int(5*fs)],fs=fs,nfft=2**13,nperseg=2048,noverlap=None,axis=1)
