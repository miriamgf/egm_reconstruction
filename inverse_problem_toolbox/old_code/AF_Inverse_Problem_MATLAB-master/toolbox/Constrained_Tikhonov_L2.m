...
function [x_hat, lambda_opt, metrics] = Constrained_Tikhonov_L2 (A, AA, L, LL, y, x, lambda1, lambda2, D, x_cons, opt_calc)
...
if length(lambda1)==1 && length(lambda2)==1
    ...
    metrics = timemetrics_mod (x(1:2039,:)', x_hat(1:2039,:)');
    ...
    if opt_calc==1  % L-Hypersurface
        ...
        if 1    % change 0 for 1 for L-Hypersurface representation.
            figure(),
    ...
    elseif opt_calc==2  % CRESO
        ...
        for i=1:length(lambda1)
            ...
            for j=1:length(lambda2)
                x_hat = (AA+lambda1(i)*LL+lambda2(j)*(D'*D))\((A'*y)+(lambda2(j)*D*x_cons));
                metrics{i,j} = timemetrics_mod (x(1:2039,:)', x_hat(1:2039,:)');
            end
        end
    end
end
