...
for m = 1:length(patFV) %#ok<NODEF>
    ...
    for n = 1:length(patFV(m).epi)
        ...
        %parametros clasicos
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        patFV(m).epi(n).DFA.fvpar_egm = df_Analysis(patFV(m).epi(n).egm,patFV(m).epi(n).fs); %#ok<AGROW>
        patFV(m).epi(n).DFA.fvpar_egm_aux = df_Analysis(patFV(m).epi(n).egm_aux,patFV(m).epi(n).fs); %#ok<AGROW>
