function fvpar = df_ECG_Analysis(ecg,fs,condiciones,byHandFlag,parfv_anterior,mensaje)
%fvpar = df_Analysis(ecg,fs,condiciones,byHandFlag,parfv_anterior,mensaje)
%----------------------------------------------------------------
% Calcula ï¿½ndices de un episodio de FV.
% Recibe un secgento de un episodio de FV, frecuencia de muestreo
...
if nargin == 2
    condiciones.w='hamming';
    condiciones.L=length(ecg);
    condiciones.R=0.5;
    condiciones.nfft=fs/0.1; %1024;
    condiciones.fh2=20; %%It was previously 15 by JL but I think it should be 20
    condiciones.diff=1;
    condiciones.dfn=1/3;
    condiciones.ro=.75;
...
drawflag1 = 1;       % representaciones grï¿½ficas de cada figure
drawflag2 = 1;       
drawflag3 = 1;
drawflag4 = 1;
normflag=1;         % normalizaciï¿½n del espectro por la potencia total
N = length(ecg);    % nï¿½mero de muestras del ecg original
t = 1/fs*(0:N-1);   % vector tiempo  



%----------------------------------
% ecg y vector de tiempos
%----------------------------------
...
[fvpar.Pecg,fvpar.f] = spectralEstimation(ecg,fs,normflag,condiciones);
...
[fvpar.Pdom,indfdom] = max(fvpar.Pecg);
fvpar.fdom = fvpar.f(indfdom);

% Frecuencia fundamental
[fvpar.f0,indff,bwfromf0,fvpar.z] = df_ECG(ecg,fs);
fvpar.Pf0 = fvpar.Pecg(indff);
% [kk,aux] = min(abs(fvpar.f-fvpar.f0));
% fvpar.Pf0 = max(fvpar.Pecg(max([1 aux-5]):aux+5));

% Armï¿½nicos superiores
[fvpar.P2nd,fvpar.P3rd,fvpar.P4th,fvpar.P5th,fvpar.f2nd,fvpar.f3rd,fvpar.f4th,fvpar.f5th] = ...
    higherHarmonic(fvpar.Pecg,fvpar.f,fvpar.f0,condiciones);

% Anchos de banda de f0 y fdom
[fvpar.bw_f0,fvpar.f0_low,fvpar.f0_up] = bandWidth(fvpar.Pecg,fvpar.f,fvpar.f0,condiciones.ro);
[fvpar.bw_fdom,fvpar.fdom_low,fvpar.fdom_up] = bandWidth(fvpar.Pecg,fvpar.f,fvpar.fdom,condiciones.ro);

% Indice de organizacion
[fvpar.oi,fvpar.ejex_oi,fvpar.ejey_oi]=organizationIndex(fvpar.Pecg,fvpar.f,fvpar.fdom,condiciones.oi);

% Frecuencia mediana
fvpar.fmedian = sum(fvpar.Pecg.*fvpar.f)/sum(fvpar.Pecg);

% Leakage
fvpar.leakage = myleakage(ecg,fs,fvpar.f0);
...
[fvpar.oi_mod,fvpar.ejex_oi_mod,fvpar.ejey_oi_mod,fvpar.ejexbase_oi_mod] = ...
    riHC(ecg,fvpar.f0,fvpar.Pecg,fvpar.f);
...
% Histograma N-alpha
fvpar.Nalpha_hist = miNalphahist(ecg,fs,drawflag4,condiciones,parfv_anterior);
...
if drawflag3    
    figure(3),clf        
    m = max(fvpar.Pecg);
    plot(fvpar.f,fvpar.Pecg,'b',fvpar.fdom,fvpar.Pdom,'xr',...
