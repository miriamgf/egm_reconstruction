...
plt.savefig('Figures/'+ str(fs_sub) + '/Batch1.png')
plt.show()
...
plt.savefig('Figures/'+ str(fs_sub) + '/Batch2.png')
plt.show()
'''
...
if Transfer_model:
    model_pretrained = transfer_learning_AE(n_layers=1)
else:
...
if Transfer_model:
    ...
    model_pretrained.compile(optimizer='adam', loss=losses.mean_squared_error, metrics=[losses.mean_squared_error, losses.mean_absolute_error])
    history = model_pretrained.fit(x_train, x_train, batch_size=1, epochs=n_epoch, validation_data=(x_val, x_val), callbacks= [callback, tensorboard_callback])
    conv_autoencoder = model_pretrained
    encoder = Latent_Space_transfer_learning(model_pretrained)
else:
    conv_autoencoder = Model(inputs=encoder.input, outputs=decoder(encoder.outputs))
    conv_autoencoder.compile(optimizer='adam', loss=losses.mean_squared_error, metrics=[losses.mean_squared_error, losses.mean_absolute_error])
    history = conv_autoencoder.fit(x_train, x_train, batch_size=1, epochs=n_epoch, validation_data=(x_val, x_val), callbacks= [callback, tensorboard_callback])
...
decoded_imgs = reshape(decoded_imgs, (len(decoded_imgs), decoded_imgs.shape[1],  decoded_imgs.shape[2], decoded_imgs.shape[3]))
decoded_imgs_train = reshape(decoded_imgs_train, (len(decoded_imgs_train), decoded_imgs_train.shape[1],  decoded_imgs_train.shape[2],decoded_imgs_train.shape[3]))
...
x_train_ls = reshape(x_train, (int(len(x_train)/n_batch),n_batch,  x_train.shape[1],  x_train.shape[2], x_train.shape[3]))
...
callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)
estimator.compile(optimizer=optimizer, loss=losses.mean_squared_error, metrics=[losses.mean_squared_error, losses.mean_absolute_error, tf.keras.metrics.RootMeanSquaredError()])
history = estimator.fit(x_train_ls, y_train, batch_size=1, epochs=n_epoch, validation_data=(x_val_ls, y_val), callbacks=[callback])
...
plt.savefig('Figures/'+ str(fs_sub) + '/Reconstruction_loss.png')
plt.show()
...
#Flatten tensors
y_test_r= reshape(y_test, (y_test.shape[0]*y_test.shape[1], y_test.shape[2]))
y_train_r= reshape(y_train, (y_train.shape[0]*y_train.shape[1], y_train.shape[2]))

estimate_egm_test_r= reshape(estimate_egms_test, (estimate_egms_test.shape[0]*estimate_egms_test.shape[1], estimate_egms_test.shape[2]))
estimate_egm_train_r= reshape(estimate_egms_train, (estimate_egms_train.shape[0]*estimate_egms_train.shape[1], estimate_egms_train.shape[2]))

#Flatten for MSE nad MAE
reconstruction_flat_test = reshape(estimate_egms_test, (estimate_egms_test.shape[0]*estimate_egms_test.shape[1], estimate_egms_test.shape[2]))
reconstruction_flat_train= reshape(estimate_egms_train, (estimate_egms_train.shape[0]*estimate_egms_train.shape[1], estimate_egms_train.shape[2]))  
y_test_flat = y_test_r
y_train_flat = y_train_r

print(estimate_egm_test_r.shape,estimate_egm_train_r.shape, y_test_r.shape)

#Save
mse_reconstruction_test, mae_reconstruction_test, rmse_reconstruction_test= results_test[0], results_test[2], results_test[3]
mse_reconstruction_train, mae_reconstruction_train, rmse_reconstruction_train= results_train[0], results_train[2], results_test[3]

#MAE and MSE
mae_reconstruction_test = mean_absolute_error(y_test_r, reconstruction_flat_test)
...
# DTW
dtw_test, path = fastdtw(estimate_egm_test_r, y_test_r)
dtw_train, path = fastdtw(estimate_egm_train_r, y_train_r)

results_reconstruction= {'mse test': mse_reconstruction_test, 'mse train': mse_reconstruction_train,
'mae test': mae_reconstruction_test, 'mae train': mae_reconstruction_train, 'rmse test': rmse_reconstruction_test, 'rmse train': rmse_reconstruction_train, 
 'dtw_test': dtw_test, 'dtw_train': dtw_train }

y_test_subsample=y_test_r
estimate_egms_test=estimate_egm_test_r

#estimate_egms_n = normalize_by_models(estimate_egm_test_r,BSPM_test)
...
if norm:
    ...
    for model in np.unique(BSPM_test):
        ...
        arr_to_norm_estimate = estimate_egm_test_r[np.where((BSPM_test== model))]     #select window of signal belonging to model i
...
for i in range(0, 20):   
    ...
    plt.ylabel('Amplitude')
    plt.title(text)
    plt.savefig('Figures/'+ str(fs_sub) + '/Reconstruction' + str(i))
    plt.show()
...
plt.ylabel('nodes')
plt.colorbar(orientation="horizontal", pad=0.2)
plt.savefig('Figures/'+ str(fs_sub) + '/2D_Rec.png')
plt.show()
...
if DF_Mapping: 
    ...
    for model in unique_test_models:
        ...
        estimation_array = estimate_egms_n[np.where((AF_models_test== model))]     #select window of signal belonging to model i
...
plt.title('Nodes')
plt.savefig('Figures/'+ str(fs_sub) + '/Correlation.png')
plt.show()
...
plt.savefig('Figures/'+ str(fs_sub) + '/Correlation_2.png')
plt.show()
...
plt.savefig('Figures/'+ str(fs_sub) + '/Corr3.png')
...
plt.legend()
plt.savefig('Figures/'+ str(fs_sub) + '/DTW.png')
...
plt.savefig('Figures/'+ str(fs_sub) + '/RMSE.png')
plt.show()
...
for height in range(0,3):
    for width in range(0,4):
        for filters in range(0,12,2):
            ...
            f, Pxx_den = scipy.signal.welch(LS_signal[:, height,width, filters], fs, nperseg=nperseg_value,noverlap = nperseg_value // 2, scaling = 'density', detrend='linear')
            plt.plot(f, Pxx_den,linewidth=0.5)
            plt.xlabel('frequency [Hz]')
            plt.ylabel('PSD [V**2/Hz]')
...
plt.savefig('Figures/'+ str(fs_sub) + '/Periodogram_rec.png')
plt.show()
...
variables = {"RMSEmean": rmse_mean, "RMSEstd": rmse_std, 'Corrmean': corr_mean, "corrstd": corr_std, 'dtwmean': dtw_mean, 'dtwstd': dtw_std,
'corrbynodes': new_correlation_array, 'rmsenodes': new_rmse_array, 'test_model_name':test_model_name, "train_model_name": train_model_name, 
"val_model_name": val_model_name}

savemat("saved_var/" + str(fs_sub) + "/reconstruction2304.mat", mdic)
savemat("saved_var/" + str(fs_sub) + "/variables23_04.mat", variables)
...
if sinusoids:
    ...
    estimator.save('saved_var/model_reconstruction.h5')
    conv_autoencoder.save('saved_var/model_autoencoder.h5')
