...
def plotting_before_AE(X_1channel, x_train, x_test, x_val, Y_model, AF_models):
    ...
    title = "500 samples from BSP from nodes {} x {} in the model {}".format(
        a, b, Y_model[s]
    )
...
def plot_Welch_periodogram(x_test, latent_vector, decoded_imgs, fs=50):
    ...
    for height in range(0, 10, 2):
        for width in range(0, 10, 2):
            ...
            f, Pxx_den = signal.welch(
                input_signal[:, height, width],
                fs,
                nperseg=len(input_signal[:, height, width]),
    ...
    for height in range(0, 10, 2):
        for width in range(0, 10, 2):
            ...
            f, Pxx_den = signal.welch(
                output_signal[:, height, width],
                fs,
                nperseg=len(output_signal[:, height, width]),
                noverlap=nperseg_value // 2,
                scaling="density",
                detrend="linear",
            )
        ...
        for width in range(0, 4):
            for filters in range(0, 12, 2):
                ...
                f, Pxx_den = signal.welch(
                    ...
                    fs,
                    nperseg=len(LS_signal[:, height, width, filters]),
                    noverlap=nperseg_value // 2,
                    scaling="density",
                    detrend="linear",
                )
