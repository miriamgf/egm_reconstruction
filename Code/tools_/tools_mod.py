...
def load_data(n_classes = 2,  SR = True, subsampling=True, fs_sub=50, SNR=20, norm=False):
    """
    Returns y values depends of the classification model
    
    Parameters:
        data_type: 'Tensor' -> tx6x4x3 tensor (used for CNN-based classification)
                   'Flat' ->  tx64 matrix (used for MLP-based classification)
        resampling -> if resampling is applied to the signals ('True', 'False')
        fs_res -> fs of the resampling (Hz)
    Returns:
        X -> input data.
        Y -> labels.
    ...
    for model_name in all_model_names: 
        ...
        for matrix in transfer_matrices:
            ...
            bsps_64= ECG_filtering(bsps_64, fs)
             ...
             # 3) RESAMPLING signal to fs= fs_sub
            if subsampling: 
                bsps_64 = signal.resample_poly(bsps_64,fs_sub,fs, axis=1)
            
            # 6) Get labels and resample to fs= fs_res
            y_labels = get_labels(n_classes, model_name)
            y_labels_list=y_labels.tolist()
            indices = [i for i, x in enumerate(y_labels_list) if x == 7]
            if len(indices)>0:
                m.append(model_name)
    ...
    if norm:
        maximum = np.max(X)
        minimum = np.min(X)
        X_norm = (X- maximum) / (maximum-minimum)
        X_norm =list(X_norm)
        X=X_norm
     
    return np.array(X),np.array(Y),np.array(Y_model)
...
def get_labels(class_type, model_name):
    """
    Get y labels for eahc classificatio type: 1, 2 or 3.

    Parameters:
      class_type:  1 -> binary classification, exist(1) or does not exist(0) driver.
                   2 -> multi-classification: does not exist(0), LA(1) and RA(2)
                   3 -> multi-classification: does not exist(0), 1-7 for each region
    ...
    if class_type == 2:
        for index, item in enumerate(labels):
            # Add sinus rhythm
            if 'Sinusal' in model_name:
                labels[index] = 0
            elif model_name.startswith("RA"):
                labels[index] = 1
            elif model_name.startswith("LA"):
                labels[index] = 2
            elif model_name == 'TwoRotors_181219':
                labels[index] = 2
