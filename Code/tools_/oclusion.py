...
class Oclussion:
    ...
    def turn_off_patches(self):
        ...
        patch_indices = {
            'P1': (slice(None), slice(0, 3), slice(0, 4)),    # B6-A4: filas 0-2, columnas 0-3 #TOP FRONT LEFT
            'P2': (slice(None), slice(3, 6), slice(0, 4)),    # B3-A1: filas 3-5, columnas 0-3 #BOTTOM FRONT RIGHT
            'P3': (slice(None), slice(0, 3), slice(6, 10)),    # D12-C4 : filas 0-2, columnas 6-8 #TOP BACK
            'P4': (slice(None), slice(3, 6), slice(6, 10)),    # D9-C1 : filas 3-5, columnas 6-8 #BOTTOM BACK
            'P5': (slice(None), slice(0, 3), slice(12, 16)),   # B6-A4: filas 0-2, columnas 8-11 #TOP FRONT RIGHT (Repetido)
            'P6': (slice(None), slice(3, 6), slice(12, 16)),   # B3-A1: filas 3-5, columnas 8-11 #BOTTOM FRONT RIGHT (Repetido)
            ...
            'P10': (slice(None), slice(3, 6), slice(10, 12))    # ILR4-R7: BOTTOM LEFT SIDE
        }
        ...
        for patch in self.patches_off:
            if patch in self.patches_off:
                new_bsps[patch_indices[patch]] = self.bsps_64[patch_indices[patch]].mean() # If patches are 0 it creates NaNs in the network
