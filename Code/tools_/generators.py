...
def preprocess_input(x_tensor,tensor_type):
    if tensor_type=='3channelTensor':
        ...
        x_interpolated = tf.keras.layers.UpSampling2D(size=(10, 10), interpolation='bilinear')(x_reshaped) 
    else:
...
#%% Generator for RNN
def generator_batches_RNN(X,Y,Y_model,data_type='Flat',val_percentage=0.2,test_percentage=0.2,input_size=50,SNR=20,number_classes = 3,val=True, shuffle_batches_train = True):
    # Check the number of models
    ...
    for model in n_models:
        ...
        if data_type=='Flat':
            ...
            test_sigs=signals[len_train+len_val:len_train+len_val+len_test,:].reshape((len_test,64,1))
        elif data_type=='1channelTensor':
            train_sigs=signals[0:len_train,:].reshape((len_train,6,16,1)) 
            val_sigs=signals[len_train:len_train+len_val,:].reshape((len_val,6,16,1))
            test_sigs=signals[len_train+len_val:len_train+len_val+len_test,:].reshape((len_test,6,16,1))
        elif data_type=='3channelTensor':
            train_sigs=signals[0:len_train,:].reshape((len_train,6,4,3)) 
            val_sigs=signals[len_train:len_train+len_val,:].reshape((len_val,6,4,3))
            test_sigs=signals[len_train+len_val:len_train+len_val+len_test,:].reshape((len_test,6,4,3))
        else:
        ...
        if data_type=='Flat':
            ...
            train_sigs=np.split(train_sigs[0:input_size*train_chunks,:,:,:],train_chunks,axis=0)
            val_sigs=np.split(val_sigs[0:input_size*val_chunks,:,:,:],val_chunks,axis=0)
            test_sigs=np.split(test_sigs[0:input_size*test_chunks,:,:,:],test_chunks,axis=0)
        elif data_type=='3channelTensor':
            train_sigs=np.split(train_sigs[0:input_size*train_chunks,:,:,:],train_chunks,axis=0)
            val_sigs=np.split(val_sigs[0:input_size*val_chunks,:,:,:],val_chunks,axis=0)
            test_sigs=np.split(test_sigs[0:input_size*test_chunks,:,:,:],test_chunks,axis=0)
        ...
        train_data=list(zip(train_sigs,tf.keras.utils.to_categorical(train_labels,num_classes=number_classes).reshape(train_chunks,1,number_classes)))
        val_data=list(zip(val_sigs,tf.keras.utils.to_categorical(val_labels,num_classes=number_classes).reshape(val_chunks,1,number_classes)))
        test_data=list(zip(test_sigs,tf.keras.utils.to_categorical(test_labels,num_classes=number_classes).reshape(test_chunks,1,number_classes)))
...
def generator_batches_autoencoder(X,Y,Y_model,data_type='Flat',val_percentage=0.2,test_percentage=0.2,input_size=50,SNR=20,val=True, shuffle_batches_train = True):
    # Check the number of models
    n_models=np.unique(Y_model) #0-139?
    ...
    if val==True:                    
        ...
        return n_batches_train,n_batches_val,n_batches_test,train_gen,val_gen,test_gen, train_list
    else:
        train_list = train_list + val_list
...
def generator_batches_RNN_kfold(X,Y,Y_model,indices,data_type='Flat',input_size=50,SNR=20,number_classes = 3,shuffle_batches_train = True):
    ...
    for model in indices:
        ...
        data=list(zip(sigs,tf.keras.utils.to_categorical(labels,num_classes=number_classes).reshape(len(sigs),1,number_classes)))
        ...
        for batch in list_data:
            ...
            if t_type=='Flat':
                ...
                for i in range(0,batch_matrix.shape[0]):
                    batch_inter.append(preprocess_input(batch_matrix[i,:,:,:],tensor_type=t_type))
...
def generator_batches(X,Y,t_indexes, batch_size = 64, data_type='Tensor',SNR=None, imgAugm=False, tensor_type='3channel'):  
    #while True:
    if SNR != None:
...
def set_generator(X_data_batches,Y_data_batches,indexes_split,data_type,imgAugm,tensor_type,batch_size):
    ...
    while True:
        if data_type!='Flat' and imgAugm==True:
            ...
            y_label=tf.keras.utils.to_categorical(Y_data_batches[chosen_instant],num_classes=8)
