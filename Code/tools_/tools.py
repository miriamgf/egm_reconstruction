...
def load_data(data_type, n_classes = 2,  SR = True, subsampling=True, fs_sub=50, SNR=20, norm=False, classification = False, sinusoid = False, n_batch=50):
    ...
    for model_name in all_model_names: 
        ...
        for matrix in transfer_matrices:
            ...
            #plot_load_data(egms, x, bsps_64_noise, bsps_64_filt, bsps_64_or, n_model, bsps_64  )
            ...
            if data_type == '3channelTensor':
                ...
                #Interpolate
                reshape_tensor = np.reshape(tensors_model, (len(tensors_model), tensors_model.shape[1], tensors_model.shape[2],1))
                tensors_model = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='bilinear')(reshape_tensor)
                tensors_model = np.reshape(tensors_model, (len(tensors_model), tensors_model.shape[1], tensors_model.shape[2]))
...
def plot_load_data(egms, x, bsps_64_noise, bsps_64_filt, bsps_64_or, n_model, bsps_64 ):
            ...
            if n_model == 160:
                ...
                plt.subplot(4, 1, 2)              
                plt.plot(y_500,bsps_64_noise[0, 0:2000],alpha= 0.75,  label='2. BSPM noise added (20 db)')
                ...
                plt.legend()
                plt.subplot(4, 1, 3)              
                plt.plot(y_500,bsps_64_noise[20, 0:2000],alpha= 0.75,  label='2. BSPM noise added (20 db)')
                ...
                plt.legend()
                plt.subplot(4, 1, 4)              
                plt.plot(y_500,bsps_64_noise[50, 0:2000], alpha= 0.75, label='2. BSPM noise added (20 db)')
...
def sinusoids_generator(n, m, fs=100):
    ...
    t = np.arange(m)/fs
    sinusoids = np.sin(2*np.pi*np.outer(t, freq) + phases) #y = A*sin (2pi * f0 *t + phase)
    return sinusoids
...
def train_test_val_split_Autoencoder(X_1channel, AF_models, BSPM_Models, all_model_names,  random_split, train_percentage, test_percentage ):
    ...
    if caution_split:
        ...
        set_models={'190619', '190717', '191001'}
        indx = []
    ...
    if random_split: 
        ...
        if caution_split:
            train_models = random.sample(list(AF_models_unique),int(np.floor(AF_models[-1]*train_percentage - len(indx))))
...
def corr_pearson_cols(array1, array2):
    ...
    assert array1.shape == array2.shape, 'Los arrays deben tener las mismas dimensiones.'
...
def DTW_by_AFModels(AF_models_test, estimate_egms_n, y_test_subsample): 
    ...
    for model in np.unique(AF_models_test):
        ...
        estimation_array = estimate_egms_n[np.where((AF_models_test== model))]     #select window of signal belonging to model i
        y_array = y_test_subsample[np.where((AF_models_test== model))]     #select window of signal belonging to model i
        random_signal= np.random.uniform(low=-1.0, high=1.0, size=(estimation_array.shape[0], estimation_array.shape[1]))
...
def interpolate_fun(array, n_models, final_nodes, sig = False ):
    ...
    if sig: 
        if array.ndim > 2:
            ...
            interpol = tf.keras.layers.UpSampling2D(size=(4, 1 ), interpolation='bilinear')(array)
...
def preprocess_latent_space(latent_vector_train, latent_vector_test, latent_vector_val,
    ...
    if dimension ==5:
        ...
        latent_vector_train=reshape(latent_vector_train, (latent_vector_train.shape[0]*latent_vector_train.shape[1], latent_vector_train.shape[2], latent_vector_train.shape[3], latent_vector_train.shape[4] ))
        latent_vector_test=reshape(latent_vector_test, (latent_vector_test.shape[0]*latent_vector_test.shape[1], latent_vector_test.shape[2], latent_vector_test.shape[3], latent_vector_test.shape[4] ))
        latent_vector_val=reshape(latent_vector_val, (latent_vector_val.shape[0]*latent_vector_val.shape[1], latent_vector_val.shape[2], latent_vector_val.shape[3], latent_vector_val.shape[4] ))
