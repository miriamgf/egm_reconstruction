import numpy as np
from tqdm import tqdm
import os
import matplotlib.pyplot as plt

from tools_.tools_1 import normalize_array


class SyntheticDataGenerator:
    def __init__(self, vae_model, latent_dim, save_dir, params, rng_seed=42, sampling="guided"):
        """
        Parameters:
        - vae_model: instancia del VAE ya cargado (con pesos).
                     Compatible con .decode_from_latent(z) o .decoder(z, training=False)
        - latent_dim: dimensión del espacio latente (int).
        - save_dir: carpeta donde se guardarán las señales generadas (.npy).
        - params: diccionario de hiperparámetros, se usa para el nombre de archivo.
        - rng_seed: semilla para reproducibilidad en muestreos.
        """
        self.vae = vae_model
        self.latent_dim = int(latent_dim)
        self.save_dir = save_dir
        os.makedirs(self.save_dir, exist_ok=True)
        self.model_name = params.get('experiment_name', 'experiment')
        self.params = params
        self.rng = np.random.default_rng(rng_seed)
        self.sampling=sampling

    # ---------------------------- Helpers API ----------------------------
    @staticmethod
    def _to_np(x):
        if isinstance(x, np.ndarray):
            return x
        try:
            return x.numpy()
        except Exception:
            return np.array(x)

    def _decode(self, z_batch):
        """
        Decodifica un batch latente z_batch -> señales.
        Intenta .decode_from_latent y si no, usa .decoder(z, training=False).
        """
        try:
            x = self.vae.decode_from_latent(z_batch)
        except Exception:
            x = self.vae.decoder(z_batch, training=False)
        return self._to_np(x)

    # ---------------------------- Sampling ------------------------------
    def sample_latent_vectors(self, num_samples, mu=None, sigma=None, z_mean_train=None):
        """
        Genera vectores latentes según el modo:
        - 'random': z ~ N(0, I)
        - 'guided': z ~ N(mu, sigma) con mu,sigma provistos (típico: estimados de z_mean_train)
        - 'interpolated': interpola linealmente entre dos z_mean reales
        """
                

        if self.sampling == "random":
            return self.rng.normal(loc=0.0, scale=1.0, size=(num_samples, self.latent_dim))

        elif self.sampling == "guided":
            if mu is None or sigma is None:
                raise ValueError("Para 'guided' debes pasar mu y sigma.")
            return self.rng.normal(loc=mu, scale=sigma, size=(num_samples, self.latent_dim))

        elif self.sampling == "interpolated":
            if z_mean_train is None or len(z_mean_train) < 2:
                raise ValueError("Para 'interpolated' necesitas z_mean_train con al menos 2 elementos.")
            idx1, idx2 = self.rng.choice(z_mean_train.shape[0], size=2, replace=False)
            z1 = z_mean_train[idx1]
            z2 = z_mean_train[idx2]
            alphas = np.linspace(0, 1, num_samples)
            return np.stack([(1 - a) * z1 + a * z2 for a in alphas], axis=0)
        
        

        else:
            raise ValueError(f"sampling desconocido: {self.sampling}")


    # ---------------------------- Decoding ------------------------------
    def decode_latent_vectors(self, z_samples, batch_size=64, squeeze=True):
        """
        Decodifica un conjunto de vectores latentes a señales sintéticas.
        Procesa en lotes para ahorrar memoria.
        """
        z_samples = np.asarray(z_samples, dtype=np.float32)
        n = z_samples.shape[0]
        out = []
        for i in range(0, n, batch_size):
            z_b = z_samples[i:i+batch_size]
            xb = self._decode(z_b)
            out.append(self._to_np(xb))
        X = np.concatenate(out, axis=0)
        if squeeze:
            X = np.squeeze(X)
        return X

    # ----------------------------- Scoring ------------------------------
    def compute_rmse(self, synthetic_signals, real_signals, chunk_size=16, show_progress=True):
        """
        Calcula el RMSE mínimo de cada señal sintética respecto al conjunto de reales.
        Vectorizado por bloques (S_chunk x R) para ser eficiente en memoria.

        synthetic_signals: (S, T, N)
        real_signals:      (R, T, N)
        return: lista de tuplas (idx_synthetic, min_rmse)
        """
        S, T, N = synthetic_signals.shape
        R = real_signals.shape[0]
        rmse_min_per_synth = []

        rng_iter = range(0, S, chunk_size)
        if show_progress:
            rng_iter = tqdm(rng_iter, total=int(np.ceil(S/chunk_size)), desc="RMSE")

        for s0 in rng_iter:
            s1 = min(s0 + chunk_size, S)
            syn = synthetic_signals[s0:s1]              # (s, T, N)
            # broadcast: (s, 1, T, N) - (1, R, T, N) -> (s, R, T, N)
            diff = syn[:, None, :, :] - real_signals[None, :, :, :]
            mse = np.mean(diff**2, axis=(2,3))          # (s, R)
            rmse = np.sqrt(mse)                         # (s, R)
            min_rmse = np.min(rmse, axis=1)             # (s,)
            rmse_min_per_synth.extend(min_rmse.tolist())

        return list(zip(range(S), rmse_min_per_synth))

    def select_best_signals(self, synthetic_signals, rmse_list, num_to_keep=100):
        """
        Selecciona las señales sintéticas con menor RMSE (más parecidas a alguna real).
        """
        rmse_list_sorted = sorted(rmse_list, key=lambda x: x[1])
        best_idxs = [idx for idx, _ in rmse_list_sorted[:num_to_keep]]
        return synthetic_signals[best_idxs]

    # ------------------------------ I/O --------------------------------
    def save_signals(self, signals, filename):
        """
        Guarda las señales sintéticas en archivo .npy.
        """
        save_path = os.path.join(self.save_dir, filename)
        np.save(save_path, signals)
        print(f"Saved {signals.shape[0]} synthetic signals at {save_path}")
    
    def plot_examples(self, signals, experiment_dir,real_signals=None, num_examples=5, max_nodes=5, prefix="synthetic"):
        """
        Guarda figuras de ejemplo de las señales generadas.

        Args:
            signals: (S, T, N) señales sintéticas.
            real_signals: (R, T, N) opcional, para comparación.
            num_examples: nº de ejemplos a graficar.
            max_nodes: nº de nodos a graficar en 1D.
            prefix: prefijo del nombre de archivo.
        """
        signals = np.asarray(signals)
        n = min(num_examples, signals.shape[0])

        for i in range(n):
            sig = signals[i]
            sig_norm=normalize_array(sig, high=1, low=-1)

            # ----- Heatmap -----
            plt.figure(figsize=(8, 4))
            plt.imshow(sig_norm, aspect="auto", cmap="viridis")
            plt.colorbar(label="Amplitude")
            plt.title(f"{prefix} example {i} (heatmap)")
            plt.xlabel("Nodes")
            plt.ylabel("Time")
            fpath = os.path.join(experiment_dir, f"{prefix}_heatmap_{i}.png")
            plt.savefig(fpath, dpi=150, bbox_inches="tight")
            plt.close()

            # ----- Comparación 1D -----
            plt.figure(figsize=(10, 6))
            for node in range(min(max_nodes, sig.shape[1])):
                plt.plot(sig_norm[:, node], label=f"Synth node {node}", alpha=0.7)

                if real_signals is not None:
                    plt.plot(real_signals[0, :, node], "--", alpha=0.5, label=f"Real node {node}")

            plt.title(f"{prefix} example {i} (1D signals)")
            plt.xlabel("Time samples")
            plt.ylabel("Amplitude")
            plt.legend(loc="upper right", fontsize="x-small")
            fpath = os.path.join(experiment_dir, f"{prefix}_1D_{i}_norm_.png")
            plt.savefig(fpath, dpi=150, bbox_inches="tight")
            plt.close()

        print(f"[INFO] Guardadas {n} figuras de ejemplos en {experiment_dir}")

    # ---------------------------- Pipeline ------------------------------
    def generate_and_select(self,
                            real_signals,
                            z_mean_train,
                            num_generated=10,
                            num_selected=5,
                            decode_batch_size=64,
                            rmse_chunk_size=16,
                            save_name=None):
        """
        Pipeline completo: samplea z, decodifica a señales, selecciona las mejores y guarda.

        Args:
            real_signals: (R, T, N) reales (normalizados igual que el decoder)
            z_mean_train: (R, D) z_mean del train (para 'guided' y 'interpolated')
            num_generated: nº de señales a generar
            num_selected: nº de señales a conservar
            sampling: 'random' | 'guided' | 'interpolated'
            decode_batch_size: batch size para decodificar
            rmse_chunk_size: tamaño de bloque para cálculo de RMSE
            save_name: nombre de archivo .npy (por defecto usa 'synt_<experiment>.npy')

        Returns:
            best_signals: array (num_selected, T, N)
        """
        # Estadísticos para modo guided
        mu = np.mean(z_mean_train, axis=0) if z_mean_train is not None else None
        sigma = np.std(z_mean_train, axis=0) if z_mean_train is not None else None

        # 1) Muestreo latente
        z_samples = self.sample_latent_vectors(
            num_generated, mu=mu, sigma=sigma, z_mean_train=z_mean_train, 
        )

        # 2) Decodificación
        synthetic_signals = self.decode_latent_vectors(z_samples, batch_size=decode_batch_size, squeeze=True)

        # 3) Scoring (RMSE vs reales) y selección
        rmse_list = self.compute_rmse(synthetic_signals, real_signals, chunk_size=rmse_chunk_size, show_progress=True)
        best_synthetic_signals = self.select_best_signals(synthetic_signals, rmse_list, num_to_keep=num_selected)

        # 4) Guardado
        if save_name is None:
            save_name = f"synt_{self.model_name}_{self.sampling}.npy"
        self.save_signals(best_synthetic_signals, save_name)

        return best_synthetic_signals
