...
x_train_batch = reshape(
    x_train,
    (int(len(x_train) / n_batch), n_batch, x_train.shape[1], x_train.shape[2], 1),
)
x_test_batch = reshape(
    x_test, (int(len(x_test) / n_batch), n_batch, x_test.shape[1], x_test.shape[2], 1)
)
x_val_batch = reshape(
    x_val, (int(len(x_val) / n_batch), n_batch, x_val.shape[1], x_val.shape[2], 1)
)

# Reshape AF_models to compute corresponding AF model to each sample
x_train_batch_AF = reshape(
    AF_models_train, (int(len(AF_models_train) / n_batch), n_batch)
)
...
decoded_imgs = reshape(
    decoded_imgs,
    (
        len(decoded_imgs),
        decoded_imgs.shape[1],
        decoded_imgs.shape[2],
        decoded_imgs.shape[3],
    ),
)
decoded_imgs_train = reshape(
    decoded_imgs_train,
    (
        len(decoded_imgs_train),
        decoded_imgs_train.shape[1],
        decoded_imgs_train.shape[2],
        decoded_imgs_train.shape[3],
    ),
)
...
# Reshape for dtw
decoded_flat = reshape(
    decoded_imgs,
    (
        decoded_imgs.shape[0] * decoded_imgs.shape[1],
        decoded_imgs.shape[2] * decoded_imgs.shape[3],
    ),
)
decoded_flat_train = reshape(
    decoded_imgs_train,
    (
        decoded_imgs_train.shape[0] * decoded_imgs_train.shape[1],
        decoded_imgs_train.shape[2] * decoded_imgs_train.shape[3],
    ),
)
x_test_flat = reshape(
...
results_autoencoder = {
    "mse test": mse_autoencoder_test,
    "mse train": mse_autoencoder_train,
    "mae test": mae_autoencoder_test,
    "mae train": mae_autoencoder_train,
    "dtw_test": dtw_test,
    "dtw_train": dtw_train,
}
...
x_test = reshape(
    x_test, (len(x_test), x_test.shape[1], x_test.shape[2], x_test.shape[3])
)
x_train = reshape(
    x_train, (len(x_train), x_train.shape[1], x_train.shape[2], x_train.shape[3])
)
...
latent_vector = reshape(
    latent_vector,
    (
        latent_vector.shape[0] * latent_vector.shape[1],
        latent_vector.shape[2],
        latent_vector.shape[3],
...
history = estimator.fit(
    ...
    batch_size=1,
    epochs=n_epoch,
    validation_data=(x_val_ls, y_val),
    callbacks=[callback],
)
...
estimate_egm_test_r = reshape(
    estimate_egms_test,
    (
        estimate_egms_test.shape[0] * estimate_egms_test.shape[1],
        estimate_egms_test.shape[2],
    ),
)
estimate_egm_train_r = reshape(
    estimate_egms_train,
    (
        estimate_egms_train.shape[0] * estimate_egms_train.shape[1],
        estimate_egms_train.shape[2],
    ),
)

# Flatten for MSE nad MAE
reconstruction_flat_test = reshape(
    estimate_egms_test,
    (
        estimate_egms_test.shape[0] * estimate_egms_test.shape[1],
        estimate_egms_test.shape[2],
...
reconstruction_flat_train = reshape(
    estimate_egms_train,
    (
        estimate_egms_train.shape[0] * estimate_egms_train.shape[1],
        estimate_egms_train.shape[2],
...
# Save
mse_reconstruction_test, mae_reconstruction_test, rmse_reconstruction_test = (
    results_test[0],
    results_test[2],
    results_test[3],
)
mse_reconstruction_train, mae_reconstruction_train, rmse_reconstruction_train = (
    results_train[0],
    results_train[2],
    results_test[3],
)
...
results_reconstruction = {
    "mse test": mse_reconstruction_test,
    "mse train": mse_reconstruction_train,
    "mae test": mae_reconstruction_test,
    "mae train": mae_reconstruction_train,
    "rmse test": rmse_reconstruction_test,
    "rmse train": rmse_reconstruction_train,
    "dtw_test": dtw_test,
    "dtw_train": dtw_train,
}
...
if norm:
    ...
    for model in np.unique(BSPM_test):
        ...
        arr_to_norm_estimate = estimate_egm_test_r[
            np.where((BSPM_test == model))
        ]  # select window of signal belonging to model i
        estimate_egms_norm = normalize_array(
            arr_to_norm_estimate, 1, -1, 0
        )  # 0: por muestas
...
if DF_Mapping:
    ...
    for model in unique_test_models:
        ...
        sig_k_label_i = interpolate_fun(
            sig_k_label.T, len(sig_k_label.T), 2048, sig=False
        )
        phase_rec_i = interpolate_fun(sig_k_rec.T, len(phase_rec.T), 2048, sig=False)
        phase_label_i = interpolate_fun(
            sig_k_label.T, len(phase_label.T), 2048, sig=False
        )
        ...
        dic_DF = {
            "DF_rec": DF_rec,
            "sig_k_rec": sig_k_rec_i,
            "phase_rec": phase_rec_i,
            "DF_label": DF_label,
            "sig_k_label": sig_k_label_i,
            "phase_label": phase_label_i,
        }
        savemat(
            "saved_var/" + str(fs_sub) + "/DF_Mapping_variables_" + str(model) + ".mat",
            dic_DF,
        )
...
dtw_array, dtw_array_random = DTW_by_AFModels(
    AF_models_test, estimate_egms_n, y_test_subsample
)
rmse_array = RMSE_by_AFModels(AF_models_test, estimate_egms_n, y_test_subsample)
correlation_array = correlation_by_AFModels(
    AF_models_test, estimate_egms_n, y_test_subsample
)
...
plt.figure(figsize=(7, 2))
# fig, ax = plt.subplots()
plt.errorbar(x, dtw_mean, yerr=dtw_std, fmt="--o", label="Test")
plt.errorbar(x, dtw_mean_random, yerr=dtw_std_random, fmt="--o", label="Random")
# plt.xticks(rotation=90)
# ax.set_xticklabels(labels)
plt.tight_layout()
plt.xlabel("AF Model")
...
interpol = tf.keras.layers.UpSampling2D(size=(4, 1), interpolation="bilinear")(
...
interpol_label = tf.keras.layers.UpSampling2D(size=(4, 1), interpolation="bilinear")(
    estimate_labels_reshaped
)
...
results_Reconstruction = pd.DataFrame.from_dict(
    results_reconstruction, orient="index", columns=["Reconstruction"]
)
