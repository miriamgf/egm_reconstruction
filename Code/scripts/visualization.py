...
data_to_read = open(dict_var_dir + "variables.pkl", "rb")
variables = pickle.load(data_to_read)
print('variables loaded...')

data_to_read = open(dict_results_dir + "dict_results_autoencoder.pkl", "rb")
dict_results_autoencoder = pickle.load(data_to_read)

data_to_read = open(dict_results_dir + "dict_results_reconstruction.pkl", "rb")
...
#Batch generation
x_train_batch= reshape(x_train_raw, (int(len(x_train_raw)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1,  x_train_raw.shape[1],  x_train_raw.shape[2],1))
x_test_batch= reshape(x_test_raw, (int(len(x_test_raw)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1,  x_test_raw.shape[1],  x_test_raw.shape[2],1))
x_val_batch= reshape(x_val_raw, (int(len(x_val_raw)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1,  x_val_raw.shape[1],  x_val_raw.shape[2],1))


# Reshape AF_models to compute corresponding AF model to each sample
x_train_batch_AF= reshape(AF_models_train, (int(len(AF_models_train)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1))
x_test_batch_AF= reshape(AF_models_test, (int(len(AF_models_test)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1))
x_val_batch_AF= reshape(AF_models_val, (int(len(AF_models_val)/TrainConfig_1.batch_size_1),TrainConfig_1.batch_size_1))
...
plt.fill_between(np.arange(TrainConfig_1.batch_size_1), pred_test[batch, :, a, b, 0], x_test[batch, :, a, b, 0], color='lightgreen')
...
pred_test_fl = reshape(pred_test,
