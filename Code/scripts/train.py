...
print("-------------EXPERIMENT 2 NETWORKS'--------------", experiment_name)
import sys
...
plt.title("Training and validation curves AE ")
plt.savefig(figs_dir + "Learning_curves_AE_P1.png")
plt.savefig(experiment_dir + "Learning_curves_AE.png")
plt.show()

# Evaluate
pred_test = conv_autoencoder.predict(
    x_test, batch_size=1
)  # x_test=[# batches, batch_size, 12, 32, 1]
pred_train = conv_autoencoder.predict(x_train, batch_size=1)  # (44, 50, 12, 32, 1)

dict_results_autoencoder = evaluate_function(
    x_train,
    x_train,
    x_test,
    x_test,
...
y_train, y_test, y_val, x_train_ls, x_test_ls, x_val_ls, n_nodes = (
    preprocessing_regression_input(
        latent_vector_train,
        latent_vector_test,
        latent_vector_val,
        train_models,
...
new_items = {
    ...
    "y_test_ls": y_test,
    "y_val_ls": y_val,
    "latent_vector_train": latent_vector_train,
    "latent_vector_test": latent_vector_test,
    "latent_vector_val": latent_vector_val,
}
dic_vars.update(new_items)
...
optimizer = Adam(lr=TrainConfig_2.learning_rate_2, clipvalue=0.8)
...
# Función para obtener las salidas intermedias
...
for i in layer_names_to_extract:
    intermediate_output = get_intermediate_output(regressor, i, x_train_ls)
    # print(intermediate_output, i)
...
plt.savefig(figs_dir + "/Reconstruction_loss_P1.png")
plt.savefig(experiment_dir + "Learning_curves_regression.png")
plt.show()


pred_test_egm = regressor.predict(x_test_ls, batch_size=1)
pred_train_egm = regressor.predict(x_train_ls, batch_size=1)

dict_results_reconstruction = evaluate_function(
    x_train_ls,
    y_train,
    x_test_ls,
    y_test,
...
new_items = {
    "pred_test_egm": pred_test_egm,
    "pred_train_egm": pred_train_egm,
    "regressor": regressor,
}
dic_vars.update(new_items)
...
if sinusoids:
    ...
    regressor.save(models_dir + "output/model/model_reconstruction_P1.h5")
    conv_autoencoder.save(models_dir + "output/model/model_autoencoder_P1.h5")
    regressor.save(experiment_dir + "model_reconstruction_P1.h5")
    conv_autoencoder.save(experiment_dir + "model_autoencoder_P1.h5")


regressor.save(
    "output/model/model_reconstruction_P1.h5"
    + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
)
conv_autoencoder.save(
    "output/model/model_autoencoder_P1.h5"
    + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
)

# Save results to csv and export
results_Autoencoder = pd.DataFrame.from_dict(
    dict_results_autoencoder, orient="index", columns=["Autoencoder"]
)
results_Reconstruction = pd.DataFrame.from_dict(
    dict_results_reconstruction, orient="index", columns=["Reconstruction"]
)
global_results = pd.concat([results_Autoencoder, results_Reconstruction], axis=1)
global_results.to_csv(dict_var_dir + "/Results_P1.csv")
global_results.to_csv(experiment_dir + "/Results_P1.csv")

global_results.round(3)

# Save dictionaries into pickle and .mat

with open(dict_var_dir + "variables_P3.pkl", "wb") as fp:
    pickle.dump(dic_vars, fp)
with open(dict_results_dir + "dict_results_reconstruction_P3.pkl", "wb") as fp:
    pickle.dump(dict_results_reconstruction, fp)
with open(dict_results_dir + "dict_results_autoencoder_P3.pkl", "wb") as fp:
...
savemat(
    dict_results_dir + "dict_results_reconstruction.mat", dict_results_reconstruction
)
