...
directory = str(os.path.dirname(current)) + "/Data4/"
...
def load_data(
    ...
    transfer_matrices = load_transfer(ten_leads=False, bsps_set=True)
...
def train_test_val_split_Autoencoder(
    X_1channel, AF_models, BSPM_Models, random_split, train_percentage, test_percentage
):
    ...
    if random_split:
        ...
        test_models = [11, 34, 1, 36, 22, 3, 20, 17]
        val_models = [x for x in aux_models if x not in test_models]
        val_models = [4, 8, 26, 31, 40]
