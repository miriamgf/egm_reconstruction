...
if y_reconstructed.shape[1] != 2048:
    ...
    interp_func_reconstructed = interp1d(x_original, y_reconstructed, axis=1, kind='cubic')
...
def egm_representation_vtk(reconstruction, label, faces, vertices, min_val, max_val, instant):
...
instant = 0
min_val, max_val = -1, 1
egm_representation_vtk(var_represent, var_represent_original, faces, vertices, min_val, max_val, instant)

# Generar video con VTK
video_filename = 'video_egm_representation_vtk.avi'
fps = 10
video_writer = cv2.VideoWriter(video_filename, cv2.VideoWriter_fourcc(*'XVID'), fps, (1920, 1080))
...
for instant in range(len(time)):
    ...
    vtk_array = vtk.util.numpy_support.vtk_to_numpy(vtk_image.GetPointData().GetScalars()).reshape(height, width, -1)
