...
class METRIC_3D_PLOTTER:
    ...
    def load_geometry_and_egm(self):
        ...
        if self.labels_mode:
            ...
            try:
                geom = sio.loadmat(self.geom_path_CF)["geometritorso_pathes"]
            except:
    ...
    def plot_3d_mesh_label(self, var_data, faces, vertices, min_val_value, max_val_value, color='red',normalizar=True ):
        ...
        custom_lut,cmap = self.create_custom_colormap(color)
        ...
        writer.SetInputData(window_to_image_filter.GetOutput())
        writer.Write()
        print(f"Saved {self.metric} map at: ", output_file)

    def create_custom_colormap(self, color):
    ...
    def extract_metric(self):
        """
        Extrae métrica de valores guardados durante evaluación
        ...
        patient_row = csv_metrics.loc[csv_metrics.iloc[:, 0] == self.model_name[0]]
