...
class Gen_VAE_2D_Skip(Model):
    ...
    def __init__(
        ...
        super(Gen_VAE_2D_Skip, self).__init__()
    ...
    def build_encoder_module(self, inputs, input_shape):
        ...
        # Encoder block 1
        x1 = layers.Conv2D(32, (5, 15), strides=(2, 4), padding='same',
        ...
        skip1 = layers.GaussianNoise(0.05)(x1)


        # Encoder block 2
        x2 = layers.Conv2D(64, (5, 15), strides=(2, 4), padding='same',
                        activation='leaky_relu', kernel_regularizer=tf.keras.regularizers.l2(self.params["l2_reg"]))(x1)
        skip2 = layers.GaussianNoise(0.05)(x2)

        # Encoder block 3
        x3 = layers.Conv2D(10, (3, 5), strides=(2, 2), padding='same',
                        activation='leaky_relu', kernel_regularizer=tf.keras.regularizers.l2(self.params["l2_reg"]))(x2)
        
        skip3 = layers.GaussianNoise(0.05)(x3)

        x_flat = layers.Flatten()(x3)
        x_dense = layers.Dense(self.latent_dim, activation='leaky_relu')(x_flat)

        z_mean = self.z_mean_dense(x_dense)
        z_log_var = self.z_log_var_dense(x_dense)
        z = self.sampling_layer([z_mean, z_log_var])
        #z = z + tf.random.normal(tf.shape(z), stddev=0.05)

        # Retornamos también los skips
        return z, z_mean, z_log_var, [skip1, skip2, skip3]



    
    def build_decoder_module(self, z, skips=None):
        if not hasattr(self, 'decoder'):

            z_input = layers.Input(shape=(self.latent_dim,), name="z_input")
            skip1 = layers.Input(shape=(200, 512, 32), name="skip1")
            skip2 = layers.Input(shape=(100, 128, 64), name="skip2")
            skip3 = layers.Input(shape=(50, 64, 10), name="skip3")
            skips_input = [skip1, skip2, skip3]

    
            x = layers.Dense(25 * 16 * 32)(z_input)
            x = layers.LeakyReLU()(x)
            x = layers.Reshape((25, 16, 32))(x)
            #x = layers.Dropout(0.2)(x)

            x = layers.Conv2DTranspose(128, (4, 4), strides=(2, 4), padding='same')(x)
            x = layers.LeakyReLU()(x)

            #if skips is not None:
                #x = layers.Concatenate()([x, skips[2]])
            x = layers.Concatenate()([x, skip3])  # x3
            #x = layers.Dropout(0.2)(x)
            ...
            self.decoder = tf.keras.Model(inputs=[z_input] + skips_input, outputs=x, name="decoder")
        ...
        if z is not None:
            if skips is not None:
                ...
                dummy_skips = [
                    tf.zeros((tf.shape(z)[0], 200, 512, 32)),
                    tf.zeros((tf.shape(z)[0], 100, 128, 64)),
                    tf.zeros((tf.shape(z)[0], 50, 64, 10))
                ]
                return self.decoder([z] + dummy_skips)

        return None

    def decode_from_latent(self, z, skips=None):
        return self.build_decoder_module(z, skips=skips)

    
    def build_autoencoder_branch(self, inputs, input_shape):

        z, z_mean, z_log_var, skips = self.build_encoder_module(inputs, input_shape)
        decoded_output = self.build_decoder_module(z, skips=skips)
    ...
    def compute_loss_VAE(self, y_pred, y_true, z_mean, z_log_var):
        ...
        beta=0.1
        total_loss = mse_loss + beta * kl_loss + 0.5 * corr_loss
