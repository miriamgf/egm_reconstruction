...
def autoencoder_model_2D(x_train):
    ...
    encoder.add(layers.Conv2D(64, (2, 2), strides=1, padding='same', activation='leaky_relu',
                              input_shape=x_train.shape[1:], kernel_regularizer=tf.keras.regularizers.l2(l=0.001),
    ...
    encoder.add(layers.MaxPooling2D((2, 2)))
    encoder.add(layers.Conv2D(12, (2, 2), strides=1, padding='same', activation='linear',
