...
class MultiOutput_VAE_skip(Model):
    ...
    def __init__(self, params, input_shape_, n_nodes, latent_dim=2, tensorboard_logs=None):
        ...
        self.decoder_conv1 = layers.Conv3D(12, (5, 2, 2), strides=1, padding="same", activation="leaky_relu", kernel_initializer=initializer)
    ...
    def build_decoder_module(self, inputs, input_shape, latent_inputs, skip_connections):
       
        # Unpack the encoder outputs to use in skip connections
        x3_skip, x7_skip = skip_connections

        # Decoder layers with skip connections from encoder
        #x1_dec = self.decoder_conv1(latent_inputs)
    ...
    def build_autoencoder_branch(self, inputs, input_shape):
        latent_space, z_mean, z_log_var, skip_connections = self.build_encoder_module(inputs, input_shape)
        decoded_output = self.build_decoder_module(inputs, input_shape, latent_space, skip_connections)
